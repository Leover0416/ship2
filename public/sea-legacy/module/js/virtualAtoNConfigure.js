...
function ownShoreList(map,admin,table,config,form) {
    ...
    _form = form;
    var ownShoreList = [];
    _admin.req('station-maintain/baseStationAPI/selectAllBaseStation', "", function (data) {
        for(var i in data){
            if(data[i].connectionStatus=="0") {
                ownShoreList.push({"shoreMmsi": data[i].shoreMmsi});
            }
        }
        _table.render({
            elem: '#ownshore-table',
...
function editVirtualAtoNInfo(data) {
    ...
    virtualAtoNLayerDetailIndex = layer.open({
        ...
        , btn: ''
        , shade: 0
        , skin: 'layui-layer-lan virtualAtoNEditLayerId'
        , area: ['755px', '570px']
        , resize: false
        , cancel: function () {
...
// 电子航标配置默认值
function virtualAtonDefaultValue() {
    $("#virtualAtonDetail-form")[0].reset();
    var data={
        virtualAtonSequence: 1,
        virtualAtonChannel: 0,
        virtualAtonSendInterval: 3,
        virtualAtonValidIdentification: 1,
        virtualAtonRepeatIndicator: 0,
        virtualAtonPositionAccuracy: 0,
        virtualAtonElectronicType: 0,
        virtualAtonType: 0,
        virtualAtonOffpositionIndicator: 0,
        virtualAtonRaimFlag: 0,
        virtualAtonAssignedModeFlag: 0,
        virtualAtonDimension: 0,
        virtualAtonTimeStamp: 0,
    };
    _form.val('virtualAtonDetail-form', data);
}